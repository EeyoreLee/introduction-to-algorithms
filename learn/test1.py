...
def calc_feat(x, i, l, r):
    ...
    计算f
    参数 : x, i, y[i-1], y[i]
    return : {("T", y[i-1], y[i]): 1, ("E", y[i], x[i]): 1}
    """
...
def calc_f(x, i, l, w, e, f):
    if (i, l) not in f:
        if i == 0:
            ...
            f[i, l] = logsumexp([
                calc_f(x, i-1, k, w, e, f) + calc_e(x, i, k, l, w, e) for k in prev_states])
    return f[i, l]
...
def calc_b(x, i, r, w, e, b):
    if (i, r) not in b:
        if i == len(x)-1:
            ...
            b[i, r] = logsumexp([
                calc_b(x, i+1, k, w, e, b) + calc_e(x, i, r, k, w, e) for k in prev_states])
    return b[i, r]
...
if __name__ == '__main__':
    # load in the corpus
    """获取语料，前后words加<S>, tags前后加0"""
    corpus = []
    test =['我_s 想_s 吃_s 西_b 瓜_e', '今_b 天_e 下_b 雨_e 了_s']
    ...
    for iternum in range(1, 50+1):
        ...
        lik = 0
        for x, y in corpus:  # x: words   y=tags
